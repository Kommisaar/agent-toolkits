...
worktree 池：项目 `AGENTS.md` 可声明 worktree 池（池目录、固定命名的池位、取用与回池规则），协调器开工时检查，未声明且将派发多任务时询问是否追加；池位跨任务复用 `node_modules` 与工具缓存，任务标识只体现在分支名。池位与依赖跨 run 保留：run 结束只注销台账登记并留痕，恢复与新 run 优先接管现存池位，物理移除仅在用户明确要求或确认不再迭代时执行。
