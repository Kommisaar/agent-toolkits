...
- 任务分支、池位或 commit SHA 确定；池位绝对路径在派发落账时必须写入任务记录；
- 基线、任务、完整或合并后门禁完成；
- reviewer 请求修改、任务阻塞、拒绝、合并或取消；
- coordinator 创建、接管或注销自有 worktree/分支；run 结束注销池位登记但物理保留
  时，追加 event 写明保留路径与各池位状态（空闲 detached / 遗留分支）。
...
6. 核对现存池位：优先接管并复用热依赖，不重装；缺失槽位才新建，依赖损坏先重装；
   池位带遗留分支时按集成分支包含性核对后处置，脏池位报告询问、不自动清理。
7. 只有 `safeToResume` 为真且用户没有新增约束时，才把状态转换为 `RUNNING` 或原先的
   `DRAINING`。
8. 确认恢复后可运行带 `--write` 的 `reconcile` 留下对账记录。
